package com.jyotirmoy.musicly.baselineprofile
...
class BaselineProfileGenerator {
    ...
    fun generate() {
        ...
        rule.collect(
            ...
            try {
                ...
                Log.d("BaselineProfileGenerator", "--- STARTING MUSICLY ---")
                ...
                Thread.sleep(6000) // Extra time for initial load
                ...
                runStep("Home Screen Extended Flow") {
                    ...
                    scrollToTop() // Go back to top to continue flow
                    Thread.sleep(1000)
                }

        
                // 2. SETTINGS (Already working)
                ...
                runStep("Settings & Scroll") {
                    ...
                    settingsBtn?.let {
                        ...
                        blindScroll()
                        device.pressBack()
                        Thread.sleep(2000) // Vital wait after returning
                    }
                }
                ...
                runStep("Library Interaction & Play") {
                    ...
                    // Click on the first song in the list (Force Play)
                ...
                runStep("Search & Genre Flow") {
                    ...
                    // We go back to Home to have the miniplayer in view
                    clickTab("Home|Inicio")
                    Thread.sleep(1500)
                }

        
                // 5. LIBRARY PAGER (Already working excellently)
                ...
                runStep("Unified Player Sheet Extended") {
                    // Go back to Home to ensure visibility
                    clickTab("Home|Inicio")
                    Thread.sleep(1500)
                    ...
                    if (miniPlayer == null) {
                        Log.w("BaselineProfileGenerator", "MiniPlayer not detected by text, using click by physical zone.")
                        // Click on the bottom center area, just above the tabs
                    ...
                    Thread.sleep(3500) // Wait for full expansion
                ...
                Log.d("BaselineProfileGenerator", "--- FLOW FINISHED ---")
                pressHome()

            } catch (e: Exception) {
                Log.e("BaselineProfileGenerator", "Fatal error: ${e.toString()}")
                e.printStackTrace()
            }
        }
    }
    ...
    private fun runStep(name: String, block: () -> Unit) {
        try {
            Log.d("BaselineProfileGenerator", ">> STEP: $name")
            block()
            Log.d("BaselineProfileGenerator", ">> OK")
        } catch (e: Exception) {
            Log.e("BaselineProfileGenerator", ">> FAILED $name: ${e.toString()}")
            e.printStackTrace()
        }
    }
