...
import com.jyotirmoy.musicly.data.model.SyncedLine
import com.jyotirmoy.musicly.data.model.SyncedWord
...
object LyricsUtils {
     ...
     * @param lyricsText The lyrics text to process.
     * @return A Lyrics object with 'plain' or 'synced' lists populated.
     */
    fun parseLyrics(lyricsText: String?): Lyrics {
        ...
        lyricsText.lines().forEach { rawLine ->
            ...
            if (lineMatcher.matches()) {
                ...
                // If we already detected that the file has synchronization and at least one SyncedLine exists,
                // treat this line as a continuation of the previous one
                if (isSynced && syncedLines.isNotEmpty()) {
                    val last = syncedLines.removeAt(syncedLines.lastIndex)
                    // Keep the previous text and add the new line with a line break.
                    val mergedLineText = if (last.line.isEmpty()) {
                        stripped
                    } else {
                        last.line + "\n" + stripped
                    }
                    // Preserve the list of synced words if it existed.
                    ...
                    // If there is no synchronization in the file, it is plain text
...
/**
 * A composable that displays a line of animated bubbles that transform
 * into musical notes when they go up and back to circles when they go down.
 *
 * @param positionFlow A flow that emits the current playback position.
 * @param time The start time for these bubbles to be visible.
 * @param color The base color for the bubbles and notes.
 * @param nextTime The end time for these bubbles to be visible.
    ...
    if (show) {
        ...
        Canvas(modifier = modifier.size(64.dp, 48.dp)) {
            ...
            for (i in 0 until bubbleCount) {
                ...
                // The scale animation is now more pronounced.
                val scale = lerpFloat(1.0f, 1.4f, morphProgress)

                // A dynamic horizontal offset is calculated that activates with morphing.
...
private fun createVectorNotePathNodes(targetSize: Float): MutableList<PathNode> {
    ...
    // Centers the path at its origin (0,0) without static corrections.
...
// --- Extension Functions for PathNode ---
