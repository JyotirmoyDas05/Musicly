...
import com.jyotirmoy.musicly.data.playlist.M3uManager
...
data class PlaylistUiState(
    ...
    // For the song selection dialog/screen
    val songSelectionPage: Int = 1, // New: to track the current selection page
    val songSelectionForPlaylist: List<Song> = emptyList(),
    val isLoadingSongSelection: Boolean = false,
    val canLoadMoreSongsForSelection: Boolean = true, // New: to know if there are more songs to load
...
class PlaylistViewModel @Inject constructor(
    ...
    companion object {
        private const val SONG_SELECTION_PAGE_SIZE =
            100 // Load 100 songs at a time for the selector
    ...
    // New function to load songs for the selector in a paginated way
        ...
        viewModelScope.launch {
            ...
            _uiState.update {
                it.copy(
                    ...
                    songSelectionPage = initialPageForLoad // Set the correct page before the call
                )
            }

            // Use the songSelectionPage from the state we just updated for the repo call
            val pageToLoad = _uiState.value.songSelectionPage // This is now the correct page
            ...
            try {
                // Collect the list of songs from the Flow on an IO thread
                ...
                // UI update is done on the main thread (default context of viewModelScope.launch)
                _uiState.update { currentStateAfterLoad ->
                    val updatedSongSelectionList = if (isInitialLoad) {
                        actualNewSongsList
                    } else {
                        // Avoid duplicates if for some reason the same page is reloaded
                        val currentSongIds =
                    ...
                    currentStateAfterLoad.copy(
                        ...
                        // Increment the page only if songs were loaded and more are expected
                        songSelectionPage = if (actualNewSongsList.isNotEmpty() && actualNewSongsList.size == SONG_SELECTION_PAGE_SIZE) {
                            ...
                            currentStateAfterLoad.songSelectionPage // Do not increment if there are no more or if the load was partial
                        }
                    )
                }
            } catch (e: Exception) {
        ...
        viewModelScope.launch {
            ...
            try {
                if (isFolderPlaylistId(playlistId)) {
                    ...
                    // Get the playlist from user preferences
                    ...
                    if (playlist != null) {
                        ...
                        // Collect the list of songs from the Flow returned by the repository on an IO thread
    ...
    private fun findFolder(
        targetPath: String,
        folders: List<com.jyotirmoy.musicly.data.model.MusicFolder>
    ): com.jyotirmoy.musicly.data.model.MusicFolder? {
        val queue: ArrayDeque<com.jyotirmoy.musicly.data.model.MusicFolder> = ArrayDeque(folders)
    ...
    private fun com.jyotirmoy.musicly.data.model.MusicFolder.collectAllSongs(): List<Song> {
