...
import com.jyotirmoy.musicly.data.database.AlbumArtThemeDao
import com.jyotirmoy.musicly.data.database.EngagementDao
import com.jyotirmoy.musicly.data.database.FavoritesDao
...
import com.jyotirmoy.musicly.data.preferences.dataStore
...
import com.jyotirmoy.musicly.data.repository.MediaStoreSongRepository
import com.jyotirmoy.musicly.data.repository.MusicRepository
import com.jyotirmoy.musicly.data.repository.MusicRepositoryImpl
import com.jyotirmoy.musicly.data.repository.SongRepository
import com.jyotirmoy.musicly.data.repository.TransitionRepository
import com.jyotirmoy.musicly.data.repository.TransitionRepositoryImpl
import com.jyotirmoy.musicly.data.repository.FolderTreeBuilder
...
object AppModule {
    ...
    @Singleton
    @Provides
    fun provideApplication(@ApplicationContext app: Context): MusiclyApplication {
        return app as MusiclyApplication
    }
    ...
    @Singleton
    @Provides
    fun provideMusiclyDatabase(@ApplicationContext context: Context): MusiclyDatabase {
        return Room.databaseBuilder(
            context.applicationContext,
            MusiclyDatabase::class.java,
            "musicly_database"
        ).addMigrations(
            MusiclyDatabase.MIGRATION_3_4,
            MusiclyDatabase.MIGRATION_4_5,
            MusiclyDatabase.MIGRATION_6_7,
            MusiclyDatabase.MIGRATION_9_10,
            MusiclyDatabase.MIGRATION_10_11,
    ...
    @Singleton
    @Provides
    fun provideAlbumArtThemeDao(database: MusiclyDatabase): AlbumArtThemeDao {
    ...
    @Singleton
    @Provides
    fun provideTransitionDao(database: MusiclyDatabase): TransitionDao {
        return database.transitionDao()
    }

    @Singleton
    @Provides
    fun provideEngagementDao(database: MusiclyDatabase): EngagementDao {
        return database.engagementDao()
    }

    @Singleton
    @Provides
    fun provideFavoritesDao(database: MusiclyDatabase): FavoritesDao {
    ...
    fun provideSongRepository(
        @ApplicationContext context: Context,
        mediaStoreObserver: com.jyotirmoy.musicly.data.observer.MediaStoreObserver,
        ...
        return OkHttpClient.Builder()
            ...
            .addInterceptor { chain ->
                ...
                val requestWithUserAgent = originalRequest.newBuilder()
                    .header("User-Agent", "Musicly/1.0 (Android; Music Player)")
                    .build()
