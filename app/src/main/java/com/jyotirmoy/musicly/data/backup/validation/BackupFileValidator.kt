package com.jyotirmoy.musicly.data.backup.validation
...
import com.jyotirmoy.musicly.data.backup.model.Severity
...
class BackupFileValidator @Inject constructor(
    ...
    fun validate(uri: Uri): BackupValidationResult {
        ...
        if (format == BackupFormatDetector.Format.UNKNOWN) {
            errors.add(ValidationError("FORMAT_UNKNOWN", "File is not a recognized Musicly backup format."))
            return BackupValidationResult.Invalid(errors)
        }
