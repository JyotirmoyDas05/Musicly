package com.jyotirmoy.musicly.ui.glancewidget.subcomponents
...
fun WavyLinearProgressIndicator(
    ...
    height: Dp = 24.dp, // Parameter to control height
    phaseShift: Float = 0f,
    ...
    // The component in Glance is simply an Image. The Bitmap is generated on demand.
    Image(
        provider = ImageProvider(
            createWavyProgressBitmap(
                context = context, // Context passed for density
                progress = progress,
                isPlaying = isPlaying,
        ...
        modifier = modifier.height(height) // Height applied to modifier
    )
}
...
private fun createWavyProgressBitmap(
    ...
    waveAmplitude: Dp,
    waveFrequency: Float,
    bitmapWidth: Int = 1000, // Fixed width for bitmap resolution
    bitmapHeight: Int
): Bitmap {
    val bmp = Bitmap.createBitmap(bitmapWidth, bitmapHeight, Bitmap.Config.ARGB_8888)
    val canvas = Canvas(bmp)

    // Use actual device density to convert Dp to Px.
    ...
    if (progress > 0) {
        if (waveAmplitudePx > 0.1f) {
            ...
            val step = 5f // Increase step to improve performance
